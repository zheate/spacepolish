...
package enum QualityEvaluationRunner {
    ...
    private static func prompt(
        ...
        let communicationPolicy = CommunicationPolicy(
        ...
        let applicationInstruction = ApplicationContextPolicy.contextInstruction(
            ...
            conversationInstruction: communicationPolicy.modelInstruction
        )
        ...
        if rewriteMode == .expand {
            let expansionPlan = ContextualExpansionPlanner.plan(
                sourceText: sourceText,
                communicationContext: CommunicationContext(
                    applicationContext: applicationContext,
                    conversationSnapshot: nil,
                    relationship: nil,
                    intent: communicationPolicy.intent,
                    voice: VoiceProfile(),
    ...
    private static func audit(
        ...
        if rewriteMode == .expand {
            ...
            let plan = ContextualExpansionPlanner.plan(
                sourceText: sourceText,
                communicationContext: CommunicationContext(
                    applicationContext: applicationContext,
                    conversationSnapshot: nil,
            ...
            contextual = ContextualExpansionGuard.audit(
                sourceText: sourceText,
                outputText: outputText,
                plan: plan
            )
